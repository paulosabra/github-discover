...
void _onRepositorySearchQueryEvent(
    RepositorySearchQueryEvent event, Emitter emit) async {
  try {
    final repositoryName = event.enteredValue.toLowerCase().trim();

    if (repositoryName.isEmpty) {
      emit(const RepositorySearchEmptyState(message: 'Type a repository name'));
    } else if (kRepositoryMock.name?.toLowerCase().startsWith(repositoryName) ??
        false) {
      emit(const RepositorySearchSuccessState(
          repository: kRepositoryMock, quantityFound: 1));
    } else {
      emit(
          const RepositorySearchErrorState(message: 'Non-existent repository'));
    }
  } catch (e) {
