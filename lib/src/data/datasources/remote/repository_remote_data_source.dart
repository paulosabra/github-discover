...
abstract class RepositoryRemoteDataSource {
  Future<List<RepositoryModel>> getRepositories({String? search});
...
class RepositoryRemoteDataSourceImpl implements RepositoryRemoteDataSource {
  ...
  @override
  Future<List<RepositoryModel>> getRepositories({String? search}) async {
    try {
      Response response =
          await dio.get('${Env.baseUrl}${Endpoint.searchRepositories}$search');
